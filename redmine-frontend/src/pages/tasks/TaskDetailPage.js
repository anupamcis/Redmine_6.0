...
import { ChevronLeft, Edit, MessageSquare, User, Calendar, Clock, Tag, Paperclip, Download, Activity } from 'lucide-react';
...
export default function TaskDetailPage() {
  ...
  // Build attachment URL with Redmine API key so browser requests
  // are authenticated without redirecting to the Redmine login page.
  ...
  const fetchTimeEntries = useCallback(async () => {
    ...
    try {
      ...
      console.error('[TaskDetailPage] Failed to load time entries:', err);
    }
  }, [taskId]);
    ...
    try {
      ...
      if (projectName) {
        try {
          ...
          console.warn('[TaskDetailPage] Project activities failed, will fallback:', projectErr);
        }
      }
      ...
      console.error('[TaskDetailPage] Failed to load time entry activities:', err);
      setTimeActivities([]);
    }
  }, [projectName]);
    ...
    <div className="w-full h-full flex flex-col bg-[var(--theme-bg)]">
      ...
      <div className="border-b border-[var(--theme-border)] bg-[var(--theme-cardBg)] px-6 py-4 sticky top-0 z-10">
        <div className="flex items-center gap-4">
          <button
            onClick={() => navigate(`/projects/${projectName}/tasks`)}
          ...
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-2">
              <h1 className="text-2xl font-semibold text-[var(--theme-text)]">{current.subject}</h1>
          ...
          <button
            onClick={() => navigate(`/projects/${projectName}/tasks/${taskId}/edit`)}
            ...
            {current.created_on && (
              <div>
                ...
                <div className="mt-1 text-sm text-[var(--theme-text)]">
                  {new Date(current.created_on).toLocaleString()}
                </div>
              </div>
            )}
          </div>
      ...
      >
        <div className="space-y-4 max-h-[70vh] overflow-y-auto">
          {current.journals && current.journals.length > 0 ? (
                ...
                {journal.details && journal.details.length > 0 && (
                  <ul className="space-y-3 text-sm text-[var(--theme-text)]">
                    {journal.details.map((detail, idx) => {
                      const isAttachment = detail.property === 'attachment' || detail.name === 'attachment';
                      let attachmentId = null;
                      let attachmentFilename = null;
                      let attachmentRecord = null;

                      if (isAttachment) {
                        const attachmentValue = detail.new_value || detail.value || '';
                        if (current.attachments && current.attachments.length > 0) {
                          const matchedAttachment = current.attachments.find(
                            (att) =>
                              att.id.toString() === attachmentValue ||
                              att.filename === attachmentValue ||
                              att.filename === detail.new_value
                          );
                          if (matchedAttachment) {
                            attachmentRecord = matchedAttachment;
                            attachmentId = matchedAttachment.id;
                            attachmentFilename = matchedAttachment.filename;
                          } else {
                            const parsedId = parseInt(attachmentValue, 10);
                            if (!isNaN(parsedId)) {
                              attachmentId = parsedId;
                              attachmentFilename = detail.new_value || attachmentValue;
                            }
                          }
                        } else {
                          const parsedId = parseInt(attachmentValue, 10);
                      ...
                      const label = formatDetailLabel(detail);
                      const key = detail.name || detail.property;
                      const oldDisplay = formatDetailValue(key, detail.old_value);
                      const newDisplay = formatDetailValue(key, detail.new_value ?? detail.value);

                      return (
                        <li key={`${journal.id}-${idx}`} className="flex flex-col gap-1">
                          <div className="font-medium text-[var(--theme-text)]">{label}</div>
                          {isAttachment && attachmentId ? (
                            <div className="flex items-center gap-2">
                              {(() => {
                                const matchedAttachment =
                                  attachmentRecord || current.attachments?.find((att) => att.id === attachmentId);
                            const fallbackView = `/attachments/${attachmentId}`;
                            const fallbackDownload = `/attachments/${attachmentId}/download`;
                            const rawView = matchedAttachment?.content_url || matchedAttachment?.url || fallbackView;
                            const rawDownload =
                              matchedAttachment?.content_url || matchedAttachment?.url || fallbackDownload;
                            const viewUrl = buildAttachmentUrl(rawView, fallbackView);
                            const downloadUrl = buildAttachmentUrl(rawDownload, fallbackDownload);
                                ...
                                return (
                                  <>
                                    ...
                                    >
                                      {attachmentFilename || detail.new_value || detail.value || 'File'}
                                    </a>
                                    <a
                                      href={downloadUrl}
                                      download
                            ...
                            <div className="text-[var(--theme-textSecondary)]">
                              changed from{' '}
                              <span className="text-[var(--theme-text)] font-medium">{oldDisplay}</span> to{' '}
                              <span className="text-[var(--theme-text)] font-medium">{newDisplay}</span>
                            </div>
                          )}
                        </li>
                      );
            ...
            <p className="text-sm text-[var(--theme-textSecondary)]">No history yet.</p>
          )}
        </div>
      </Modal>
    </div>
