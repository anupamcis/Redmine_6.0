...
module ApplicationHelper
            ...
            ((avatar(principal, :size => 16).presence if principal.is_a?(User)) ||
               content_tag(
                 ...
                 :class => "name icon icon-#{principal.class.name.downcase}"
                          ...
                          :span,
                          principal.to_s,
                          title: (principal.is_a?(User) && !(!principal.company.nil? && !principal.company.default_company) ? "#{principal}(#{principal.mail})" : principal.company.name)
                        )
        )
    end
    s.html_safe
  ...
  def page_header_title
    if ((@project.nil? || @project.new_record?) && params[:context_id].nil?) && !(params[:id] == 'global-permissions')
      h(Setting.app_title)
    else
      b = []
      if (@project.nil? && params[:id] == 'global-permissions') || params[:context_id] == 'global-permissions'
