...
class UserPreference < ApplicationRecord
  ...
  def initialize(attributes=nil, *args)
    ...
    # Ensure others is always a Hash
  ...
  def set_others_hash
    # Ensure others is always a Hash, even if corrupted data exists
    unless others.is_a?(Hash)
      self.others = {}
    end
    self.others ||= {}
  ...
  def [](attr_name)
    if has_attribute? attr_name
      ...
      if others.is_a?(Hash)
        ...
        # If others is not a Hash (e.g., corrupted data), initialize it as one
        self.others = {}
        nil
      end
    end
  ...
  def []=(attr_name, value)
    if has_attribute? attr_name
      ...
      # Ensure we have a Hash, even if corrupted data exists
      current_others = read_attribute(:others)
      h = if current_others.is_a?(Hash)
            current_others.dup
          else
            {}
          end
