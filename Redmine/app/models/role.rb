...
class Role < ApplicationRecord
  ...
  # Returns true if the role has any user-management related permission.
  # Some plugins (eg. member_author) expect this helper.
  def has_user_management_permissions?
    return false unless permissions.is_a?(Array)

    management_perms = [
      :add_manager,          # custom/project-manager style permission
      :manage_members,       # core Redmine permission
      :manage_users          # core/admin permission
    ]

    (permissions.map(&:to_sym) & management_perms).any?
  end
