...
class Issue < ApplicationRecord
  ...
  validate :notes_required_when_moving_to_status_15
  ...
  def notes_required_when_moving_to_status_15
    if will_save_change_to_status_id? &&
      status_id.to_i == 15 &&
      (current_journal.nil? || current_journal.notes.to_s.strip.blank?)

      errors.add(:base, "Please enter a note when changing the status to #{status.name}.")
    end
  end
