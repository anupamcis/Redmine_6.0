...
class RepositoriesController < ApplicationController
  ...
  before_action :find_project_by_project_id, :only => [:new, :create, :index]
  ...
  accept_api_auth :add_related_issue, :remove_related_issue, :index, :create, :update, :destroy

  rescue_from Redmine::Scm::Adapters::CommandFailed, :with => :show_error_command_failed

  def index
    # Allow viewing repositories if user can view the project
    unless @project && (User.current.allowed_to?(:view_repository, @project) || User.current.allowed_to?(:view_project, @project))
    ...
    respond_to do |format|
      format.html { redirect_to settings_project_path(@project, :tab => 'repositories') }
      format.api do
        @repositories = @project.repositories.order(:is_default => :desc, :identifier => :asc).to_a
        render json: @repositories.map { |repository|
          {
            id: repository.id,
  ...
  def create
    ...
    if @repository.save
      ...
      respond_to do |format|
        ...
        format.api { render_validation_errors(@repository) }
      end
    end
  end
