...
class IssuesController < ApplicationController
  ...
  before_action :find_issue, :only => [:edit, :update, :issue_tab]
  before_action :find_issue_with_eager_load, :only => [:show]
    ...
    if @query.valid?
      respond_to do |format|
        format.html do
          ...
          # Performance fix (FINDING-001 + Caching): Cache query results and force eager loading
          ...
          cache_key = "issues/index/#{query_hash}/#{@issue_pages.page}/#{User.current.id}/#{per_page_option}"
          
          # Use Rails cache for query results (2 minute TTL)
          # Cache invalidation: automatic after 2 minutes, or manual via Rails.cache.clear
          @issues = Rails.cache.fetch(cache_key, expires_in: 2.minutes, race_condition_ttl: 10.seconds) do
            # Performance fix (FINDING-001): Force eager loading of ALL commonly accessed associations
            # IssueQuery only conditionally preloads based on visible columns, but views may access others
            # So we get the issue IDs first, then reload with full eager loading
            issue_ids = @query.issue_ids(
              :offset => @issue_pages.offset, 
              :limit => @issue_pages.per_page
            )
            
            # Force eager load all associations to prevent any N+1 queries
            ...
            # Batch load relations and other computed data
            Issue.load_visible_relations(issues) if issues.any?
            Issue.load_visible_last_updated_by(issues) if issues.any?
  ...
  # Performance fix (FINDING-002): Optimized find_issue for show action with eager loading
  def find_issue_with_eager_load
    # Eager load all commonly accessed associations from the start to prevent N+1
    ...
    # Preload relations efficiently - load_visible_relations does batch loading
    Issue.load_visible_relations([@issue]) if @issue.present?
  rescue ActiveRecord::RecordNotFound
    render_404
  end

  def show
    # Performance fix (FINDING-002): Issue is already eager-loaded in find_issue_with_eager_load
    # Fragment caching is handled in views for better granularity
    ...
    respond_to do |format|
      format.html do
        ...
        # Performance: time_entries already preloaded, apply visible filter in memory
        @time_entries = @issue.time_entries.to_a.select { |te| te.visible?(User.current) rescue true }
        @relation = IssueRelation.new
        # Performance: changesets already preloaded via includes, use loaded data
        # .visible scope is applied by Changeset, but we check loaded collection to avoid query
        preloaded_changesets = @issue.changesets.to_a
        @has_changesets = preloaded_changesets.any? { |cs| cs.repository&.project && cs.repository.project.active? }
