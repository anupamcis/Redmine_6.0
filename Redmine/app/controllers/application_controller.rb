...
class ApplicationController < ActionController::Base
  ...
  before_action :handle_cors_preflight, if: -> { request.options? }
  ...
  def require_login
    logger.info("DEBUG require_login: User.current.logged? = #{User.current.logged?}")
    unless User.current.logged?
      ...
      logger.info("DEBUG require_login: Returning false (user not logged)")
      return false
    end
    logger.info("DEBUG require_login: User is logged, params[:id] = #{params[:id]}")
    if params[:id] == 'global_permissions'
      logger.info("DEBUG require_login: Returning false (global_permissions)")
      return false
    end
    logger.info("DEBUG require_login: Returning true")
    true
  end
  ...
  def authorize(ctrl = params[:controller], action = params[:action], global = false)
    if @project.nil? && (params[:id] == 'global-permissions' || params[:context_id] == 'global_permissions')
      @project = Project.find_by(:name => l(:global_permissions))
    end
