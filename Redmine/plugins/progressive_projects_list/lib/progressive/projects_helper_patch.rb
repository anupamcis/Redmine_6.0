    ...
    base.class_eval do
      ...
      def render_project_issues_count(project, open_issues=nil)
        ...
        if open_issues.any?
          ...
          s << due_date_tag(project.opened_due_date) if project.respond_to?(:opened_due_date) && project.opened_due_date && (User.current.respond_to?(:company_id) && User.current.company_id.nil? || (User.current.respond_to?(:company) && User.current.company&.default_company))
          s << "</div>"
      ...
      def render_project_progress_bars(project)
        ...
        if open_issues.present?
          issues_closed_percent = project.issues_closed_percent
          s << "<div>" + l(:label_issue_plural) + ": " +
            link_to(l(:label_x_open_issues_abbr, :count => open_issues.size), :controller => 'issues', :action => 'index', :project_id => project, :set_filter => 1) +
            " <small>(" + l(:label_total) + ": #{project.issues.count})</small> "
          s << due_date_tag(project.opened_due_date) if project.opened_due_date
          s << "</div>"
          s << progress_bar([issues_closed_percent, project.issues_completed_percent], :width => '30em', :legend => ('%0.0f%%' % (issues_closed_percent || 0)))
        end
        if open_versions.present?
          s << "<div>"
          open_versions.reverse_each do |version|
            ...
            s << "<br>" +
              progress_bar([version.closed_percent, version.completed_percent], :width => '30em', :legend => ('%0.0f%%' % (version.completed_percent || 0)))
          end
          s << "</div>"
        end
        s.html_safe
