module FavoriteProjectsHelper
  ...
  def favorite_css(objects)
    ...
    "#{objects.first.class.to_s.underscore}-#{id}-favorite"
  end
  ...
  def favorite_project_query_links(title, queries, object_type)
    ...
    url_params = if controller_name == "#{object_type}s"
      {:controller => "#{object_type}s", :action => 'index'}
    else
      # Strong parameters: convert to a safe hash for URL building
    ...
    content_tag('h3', title) + "\n" +
      content_tag('ul',
        queries.collect {|query|
            ...
            content_tag('li', link_to(query.name, url_params.merge(:query_id => query.id), :class => css))
          }.join("\n").html_safe,
        :class => 'queries'
      ) + "\n"
  end
