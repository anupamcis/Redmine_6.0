class FavoriteProjectsQuery < Query
  ...
  self.operators_by_filter_type[:list_project] = [ "=", "!", "*" ]
  ...
  def initialize_available_filters
    ...
    add_available_filter "status",
      :type => :list_project, :values => IssueStatus.sorted.collect{|s| create_status_filter_values(s.name) if ['New', 'Resolved', 'In Progress', 'Feedback', 'Rejected', 'Closed'].include?(s.name)}.compact
    ...
    if User.current.admin?
      if Project.respond_to?(:available_tags)
        tags_source = Project.available_tags
      end
    else
      if defined?(RedmineCrm) && defined?(RedmineCrm::Tagging)
        tags_source = RedmineCrm::Tagging.where(taggable_id: User.current.projects.map(&:id)).map(&:tag)
      end
    end
    tags = tags_source.collect { |t| [t.name, t.name] }
    ...
    if tags.present?
      if !User.current.respond_to?(:company_id) || User.current.company_id.nil? || (User.current.respond_to?(:company) && User.current.company&.default_company)
        add_available_filter "tags", :type => :list, :values => tags, :order => 8
      end
    end
  ...
  def available_columns
    ...
    if !User.current.respond_to?(:company_id) || User.current.company_id.nil? || (User.current.respond_to?(:company) && User.current.company&.default_company)
      @available_columns << QueryColumn.new(:tags, :caption => :label_favorite_project_tags_plural)
    end
  ...
  def sql_for_is_favorite_field(field, operator, value)
    if User.current.logged?
      compare  =  if operator == '=' && value.try(:first) == "1"
        'IN'
      elsif operator == '=' && value.try(:first) == "0"
        'NOT IN'
      elsif operator == '!' && value.try(:first) == "1"
        'NOT IN'
      else
        'IN'
      end
  ...
  def create_status_filter_values(status)
    case status
    when 'New'
      ['Active', Project::STATUS_ACTIVE.to_s]
    when 'In Progress'
      ["Closed", Project::STATUS_CLOSED.to_s]
    when 'Resolved'
      ["Archived", Project::STATUS_ARCHIVED.to_s]
    when 'Feedback'
      ["Scheduled for deletion", Project::STATUS_SCHEDULED_FOR_DELETION.to_s]
    when "Rejected"
      ["Scheduled for deletion", Project::STATUS_SCHEDULED_FOR_DELETION.to_s]
    else
      ["Active", Project::STATUS_ACTIVE.to_s]
