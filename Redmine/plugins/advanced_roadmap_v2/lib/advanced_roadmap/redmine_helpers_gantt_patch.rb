  ...
  module RedmineHelpersGanttPatch
    def self.included(base)
      base.class_eval do
        ...
        if instance_methods.include?(:render_version)
          alias_method :render_version_without_milestones, :render_version
          ...
          if milestone.is_a?(Milestone) && milestone.effective_date
            ...
            when :html
              if Redmine::VERSION::MAJOR == 2
                ...
                html_task(options, coords, true, label, nil)
              end
            when :image
              if Redmine::VERSION::MAJOR == 2
                image_task(options, coords, :label => label, :markers => true, :height => 3)
              else
                image_task(options, coords, true, label, nil)
              end
            when :pdf
              if Redmine::VERSION::MAJOR == 2
                pdf_task(options, coords, :label => label, :markers => true, :height => 0.8)
              else
                pdf_task(options, coords, true, label, nil)
              end
            end
          else
