    ...
    def self.included(base)
      base.class_eval do
        ...
        def link_to_milestone(milestone)
          return(link_to(milestone.name, {:controller => :milestones, :action => :show, :id => milestone.id}, :title => format_date(milestone.effective_date)))
        end
        ...
        def total_graph_tag(versions, totals, options = {})
          small_width = options[:small_width] || 500
          small_height = options[:small_height] || 400
