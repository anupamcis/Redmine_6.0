...
require_relative '../../lib/redmine_agile/helpers/agile_helper'
...
class AgileBoardsController < ApplicationController
  ...
  def update
    ...
    saved = params[:issue] && params[:issue].respond_to?(:to_unsafe_h) &&
      params[:issue].to_unsafe_h.inject(true) do |total, (attr_key, attr_val)|
        total && (@issue.attributes[attr_key].to_i == attr_val.to_i)
      end
