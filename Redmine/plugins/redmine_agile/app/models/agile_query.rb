...
class AgileQuery < Query
  ...
  self.available_columns = [
    ...
    QueryColumn.new(:fixed_milestone, :groupable => true)
  ]
  ...
  def initialize_available_filters
    ...
    if versions.any? && (User.current.allowed_to?(:manage_versions, nil, global: true) || User.current.admin?)
      add_available_filter "fixed_version_id",
        :type => :list_optional,
        :values => versions.sort.collect{|s| ["#{s.project.name} - #{s.name}", s.id.to_s] }
    end

    if milestones.any? && (User.current.allowed_to?(:manage_milestones, nil, global: true) || User.current.admin?)
  ...
  def groupable_columns
    available_columns.select {|c| (c.respond_to?(:groupable?) ? c.groupable? : c.groupable) && !c.is_a?(QueryCustomFieldColumn)}
  end
