...
# require 'SVG/Graph/Plot' # Commented out - missing dependency

module SVG
  module Graph
    # Stub class for missing SVG::Graph::Plot dependency
    class Plot
    ...
    class AgileTimeSeries < Plot
      ...
      # Minimal implementation to avoid dependency on svg-graph gem
      def add_data(data)
        @data ||= []
        raise ArgumentError, 'data must be a Hash with :data key' unless data.is_a?(Hash) && data[:data].is_a?(Array)
        @data << data
      end

      # Generate a minimal valid SVG so pages render without the optional gem
      def burn
        width  = (@config && @config[:width])  || 800
        height = (@config && @config[:height]) || 400
        %Q(<svg xmlns="http://www.w3.org/2000/svg" width="#{width}" height="#{height}"></svg>)
      end
...
# Compatibility alias for Zeitwerk (expects Svg from lib/svg path)
module Svg
  module Graph
    AgileTimeSeries = SVG::Graph::AgileTimeSeries
  end
end
