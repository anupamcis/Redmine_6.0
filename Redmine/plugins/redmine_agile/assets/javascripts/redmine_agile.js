      ...
      $issuesCols.sortable({
        ...
        stop: function(event, ui) {
          ...
          $.ajax({
            ...
            data: params,
            success: function(data, status, xhr) {
              $column.find('.issue-card').removeClass("status" + oldStatusId )
              $column.find('.issue-card').addClass("status" + newStatusId )
              self.successSortable(oldStatusId, newStatusId, oldSwimLaneId, swimLaneId);
            },
            error: function(xhr, status, error) {
              $column.find('.issue-card').removeClass("status" + newStatusId )
              $column.find('.issue-card').addClass("status" + oldStatusId )
              self.errorSortable($oldColumn, xhr.responseText);
            }
          });
        }
