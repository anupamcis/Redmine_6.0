...
class SprintsController < ApplicationController
  ...
  before_action :authorize_scrum_module, :only => [:index]
  ...
  def index
    if (current_sprint = @project.current_sprint)
      ...
      # Check if there are any sprints at all
      if @project.sprints_and_product_backlog.empty?
        # No sprints exist - redirect to settings with helpful message
        flash[:error] = l(:error_no_sprints)
        redirect_to settings_project_path(@project, :tab => 'sprints')
      else
        # There are sprints but no current one - redirect to settings
        flash[:warning] = l(:error_no_current_sprint)
        redirect_to settings_project_path(@project, :tab => 'sprints')
      end
    end
  rescue => e
    Rails.logger.error "SprintsController#index error: #{e.class} - #{e.message}"
    Rails.logger.error e.backtrace.join("\n") if Rails.env.development?
    flash[:error] = "Error loading sprints: #{e.message}"
...
private
  ...
  def authorize_scrum_module
    # Check if scrum module is enabled for the project
    unless @project.module_enabled?(:scrum)
      Rails.logger.warn "Scrum module not enabled for project #{@project.identifier}" if Rails.env.development?
      render_404
      return false
    end
    # Check permissions - view_sprint_board is required for index action
    unless User.current.allowed_to?(:view_sprint_board, @project)
      Rails.logger.warn "User #{User.current.id} doesn't have view_sprint_board permission for project #{@project.identifier}" if Rails.env.development?
      deny_access
      return false
    end
    true
  rescue => e
    Rails.logger.error "authorize_scrum_module error: #{e.class} - #{e.message}"
    Rails.logger.error e.backtrace.join("\n") if Rails.env.development?
    render_404
    false
  end
