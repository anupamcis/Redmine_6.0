...
class ScrumController < ApplicationController
  ...
  before_action :find_issue, :only => [:change_story_points, :change_pending_effort, :change_assigned_to,
                                       :new_time_entry, :create_time_entry, :edit_task, :update_task,
                                       :change_pending_efforts]
  before_action :find_sprint, :only => [:new_pbi, :create_pbi]
  before_action :find_pbi, :only => [:new_task, :create_task, :edit_pbi, :update_pbi, :move_pbi,
                                     :move_to_last_sprint, :move_to_product_backlog]
  before_action :find_project_by_project_id, :only => [:release_plan, :stats]
  before_action :authorize, :except => [:new_pbi, :create_pbi, :new_task, :create_task,
                                        :new_time_entry, :create_time_entry,
                                        :change_pending_efforts]
  before_action :authorize_add_issues, :only => [:new_pbi, :create_pbi, :new_task, :create_task]
  before_action :authorize_log_time, :only => [:new_time_entry, :create_time_entry]
  before_action :authorize_edit_issues, :only => [:change_pending_efforts]
