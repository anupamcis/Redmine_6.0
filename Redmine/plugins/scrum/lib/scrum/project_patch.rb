  ...
  module ProjectPatch
    def self.included(base)
      base.class_eval do
        ...
        # Use sprints_and_product_backlog as base to avoid is_product_backlog column dependency
        # Filter in methods themselves based on column existence
        ...
        # Dynamic sprints association that filters by is_product_backlog if column exists
        def sprints
          if has_is_product_backlog_column?
            sprints_and_product_backlog.where(:is_product_backlog => false)
          else
            sprints_and_product_backlog
          end
        end
        ...
        # Helper method to check if column exists (cached per request)
        def has_is_product_backlog_column?
          @has_is_product_backlog_column ||= begin
            Sprint.column_names.include?('is_product_backlog')
          rescue
            false
          end
        end
