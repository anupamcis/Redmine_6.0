class DailyStatusWatchersController < ApplicationController
  ...
  def create
    if request.post?
      ...
      hash_param = { watchable_type: 'DailyStatusSetting', watchable_id: @watched.id, user_id: params[:user_id] }
      watcher = Watcher.find_by(hash_param)
      if watcher.present?
        @watched.set_watcher(User.find(params[:user_id]), false)
      else
        Watcher.create(watchable_type: 'DailyStatusSetting', watchable_id: @watched.id, user_id: params[:user_id])
      end
    end
