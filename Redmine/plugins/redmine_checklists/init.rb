...
# Configure Zeitwerk to ignore the compatibility/2.1 directory (invalid constant name "2.1")
if defined?(Rails) && defined?(Rails.autoloaders)
  compatibility_path = File.join(__dir__, 'lib', 'redmine_checklists', 'patches', 'compatibility', '2.1')
  Rails.autoloaders.main.ignore(compatibility_path) if Dir.exist?(compatibility_path)
end

require File.expand_path('lib/redmine_checklists/redmine_checklists', __dir__)
