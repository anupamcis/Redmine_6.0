...
module ScmRepositoriesHelperPatch
  ...
  def self.included(base)
    ...
    base.class_eval do
      ...
      alias_method :subversion_field_tags_without_add, :subversion_field_tags
      alias_method :subversion_field_tags, :subversion_field_tags_with_add
      alias_method :mercurial_field_tags_without_add, :mercurial_field_tags
      alias_method :mercurial_field_tags, :mercurial_field_tags_with_add
      alias_method :git_field_tags_without_add, :git_field_tags
      ...
      alias_method :scm_select_tag_without_add, :scm_select_tag
      alias_method :scm_select_tag, :scm_select_tag_with_add
    end
  end
  ...
  module InstanceMethods
    ...
    def github_field_tags(form, repository)
      if params[:repository_scm].eql?("Other")
        urltag = form.text_field(:url, :size => 60,
         :required => true,
         :disabled => !repository.safe_attribute?('url'))
      else
        urltag = form.select("url", @repository.get_gitlab_repo, { include_blank: 'Select repository', :required => true}, class: "repository_scm_drop select2")
      end
      if repository.new_record? && GithubCreator.enabled? && !limit_exceeded
        ...
        note = params[:repository_scm].eql?("Other") ? l(:text_github_repository_note_new) : l(:text_gitlab_repository_note_new)
      end

      githubtags  = content_tag('p', urltag + '<br />'.html_safe + note)
      githubtags << content_tag('p', form.text_field(:login, :size => 30,required: true)) +
      content_tag('p', form.password_field(:password, :size => 30,
        :name => 'ignore',required: true,
        :value => ((repository.new_record? || repository.password.blank?) ? '' : ('x'*15)),
        :onfocus => "this.value=''; this.name='repository[password]';",
        :onchange => "this.name='repository[password]';") +
      '<br />'.html_safe + l(:text_github_credentials_note)) if params[:repository_scm].eql?("Other")
    ...
    def scm_select_tag_with_add(repository)
      ...
      Redmine::Scm::Base.all.each do |scm|
        if Setting.enabled_scm.include?(scm) ||
          ...
          scm_options << ["Other", "Other"]
        end
      end
      select_tag('repository_scm',
       options_for_select(scm_options, repository.new_record? ? params[:repository_scm] : check_repositroy(repository)),
       :disabled => (repository && !repository.new_record?),
       :data => {:remote => true, :method => 'get', url: new_project_repository_path(@project)})
    end
    private
    ...
    def check_repositroy(repository)
      if repository.url.match("github.com").present? || repository.url.match("bitbucket.org").present?
        "Other"
      else
        repository.class.name.demodulize
      end
    end
  end
