...
class CodeReviewController < ApplicationController
    ...
    begin
      CodeReview.transaction {
        ...
        if params[:issue] and params[:issue][:tracker_id]
          ...
          @review.issue.tracker_id = Tracker.find_by_name(Setting.plugin_redmine_code_review['tracker_id'].to_s).try(:id)
        end
  ...
  def assign
    ...
    issue[:tracker_id] = Tracker.find_by_name(Setting.plugin_redmine_code_review['assignment_tracker_id'].to_s).try(:id)
