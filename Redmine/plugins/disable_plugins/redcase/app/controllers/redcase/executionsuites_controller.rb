...
class Redcase::ExecutionsuitesController < ApplicationController
	...
	def index
		if params[:get_results].nil?
			...
			@environment = ExecutionEnvironment.find(params[:environment_id]) rescue nil
			@version = Version.find(params[:version_id]) rescue nil
	...
	def create
		execution_suite =
			...
			execution_suite.name = execution_suite.name.strip
