...
module DmsfHelper
  ...
  def self.all_children_sorted(parent, pos, ident)
    # Folders && files && links
    nodes = if User.current.is_client?
      parent.dmsf_folders.visible.visible_to_client + parent.dmsf_links.visible + parent.dmsf_files.visible.visible_to_client
    else
      parent.dmsf_folders.visible + parent.dmsf_links.visible + parent.dmsf_files.visible
    end
  ...
  def self.dmsf_to_csv(parent, columns, folder_and_file_ids=nil)
    #Redmine::Export::CSV.generate do |csv|
    @folder_and_file_ids = folder_and_file_ids
  ...
  def self.object_to_csv(csv, parent, columns, level = -1)
    ...
    # Sub-folders
    if @folder_and_file_ids.present?
      folders = level>=0 ? parent.dmsf_folders.visible.to_a : parent.dmsf_folders.where(id: @folder_and_file_ids).to_a
      files = level>=0 ? parent.dmsf_files.visible.to_a : parent.dmsf_files.where(id: @folder_and_file_ids).to_a
    else
