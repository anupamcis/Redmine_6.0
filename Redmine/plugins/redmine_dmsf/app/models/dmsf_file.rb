...
begin
  ...
  # Xapian is optional - DMSF works without it (full-text search disabled)
  # Only log at debug level to avoid showing as error (it's expected behavior)
  Rails.logger.debug 'DMSF: Xapian bindings not available - full-text search will be disabled. This is optional and DMSF will work normally without it.'
...
class DmsfFile < ActiveRecord::Base
  ...
  scope :visible_to_client, -> { where(is_visible_to_client: true)}
  ...
  #safe_attributes :project, :project_id
