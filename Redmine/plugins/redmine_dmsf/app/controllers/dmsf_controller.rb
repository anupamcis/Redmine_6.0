...
class DmsfController < ApplicationController
  ...
  def show
    ...
    respond_to do |format|
      ...
      format.csv  {
        ...
        send_data(DmsfHelper.dmsf_to_csv(@folder ? @folder : @project, params[:settings][:dmsf_columns], params[:folder_ids].split(",")),
  ...
  def create
    ...
    @folder.is_visible_to_client = params[:dmsf_folder][:is_visible_to_client]
  ...
  def generate_csv(dmsf_folders)
    CSV.generate(headers: true) do |csv|
      csv << params[:settings][:dmsf_columns]

      dmsf_folders.each do |folder|
        rows = []
        params[:settings][:dmsf_columns].each do |column|
          rows.push(folder[column])
        end
        csv << rows # Change these to match the actual folder attributes
      end
    end
  end
end
