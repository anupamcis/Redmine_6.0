...
# Configure Zeitwerk to ignore the test directory (test files shouldn't be autoloaded)
if defined?(Rails) && defined?(Rails.autoloaders)
  test_path = File.join(__dir__, 'lib', 'redmine_dmsf', 'test')
  Rails.autoloaders.main.ignore(test_path) if Dir.exist?(test_path)
end

require_relative 'lib/redmine_dmsf'
require 'zip'
...
Redmine::Plugin.register :redmine_dmsf do
  ...
  Redmine::Activity.register :dmsf_file_revision_accesses
