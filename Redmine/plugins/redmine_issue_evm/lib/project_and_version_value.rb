...
module ProjectAndVersionValue
  ...
  SQL_COM = 'issues.start_date IS NOT NULL AND due_date IS NOT NULL'
  # Baselines.
  ...
  def incomplete_project_issues(proj, basis_date)
    Issue.cross_project_scope(proj, 'descendants')
      .where("#{SQL_COM} AND issues.start_date <= ? AND (closed_on IS NULL OR closed_on > ?)", basis_date, basis_date.end_of_day)
  end
