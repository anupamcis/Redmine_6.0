  ...
  module Preparation
    def self.prepare
      ...
      MenuManager.map :admin_menu do |menu|
        ...
        menu.push :notification_email_templates, {:controller => 'notification_email_templates', :action => 'index'},
                  :caption => :label_notification_email_templates,
                  :icon => 'email',
      ...
      MenuManager.map :project_menu do |menu|
        menu.push(
          ...
          :if => Proc.new {|p| Setting.new_item_menu_tab == '2' && User.current.admin? },
          :html => {:id => 'new-object',
